#!/usr/bin/env python3
"""Verify that published localized documents were reviewed against exact English source bytes.

Localized long-form documents declare `reviewedSourceBlob` beside `sourceRepoSource`
in site/i18n/locales.json. The value is the Git blob SHA of the exact canonical
English file that was reviewed. Any later change to that English file changes the
blob SHA and fails this verifier until the translation is reviewed again.
"""
...
def verify(repo_root: Path, registry: dict[str, object]) -> int:
...
def main() -> None:
    ...
    checked = verify(repo_root, load_registry(repo_root))
