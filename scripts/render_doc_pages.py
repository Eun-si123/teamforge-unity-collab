...
def language_nav(registry: dict[str, object], route: str, active: dict[str, object]) -> str:
    ...
    untranslated_template = str(ui.get("untranslatedHomeTitle") or "{label} homepage; this document is not translated yet")
    parts: list[str] = []
    for locale in locales(registry, published_only=True):
        ...
        if code == active_code:
            part = f'<strong lang="{html_lang}" translate="no">{label}</strong>'
        else:
            ...
            part = (
                f'<a href="{html.escape(target, quote=True)}" lang="{html_lang}" '
                f'hreflang="{hreflang}" translate="no"{title}>{label}</a>'
            )
        if parts:
            parts.append('<span aria-hidden="true"> · </span>')
        parts.append(part)
    return f'<span class="language-switch" aria-label="{aria}">' + "".join(parts) + "</span>"
