...
저는 친구와 Unity 게임을 만들고 있었습니다. 둘이 같은 프로젝트를 함께 작업하고 싶었지만, 실제 과정은 생각보다 훨씬 불편했습니다. 프로젝트 상태를 계속 주고받고, 파일을 기다리고, 누가 무엇을 바꿨는지 맞춰 가고, 서로 같은 Project state를 유지하는 과정이 번거로웠습니다.

그래서 처음 TeamForge를 만들기 시작한 이유는 단순했습니다.

**친구와 제가 같이 쓰기 편한 도구가 필요했기 때문입니다.**

그런데 만들다 보니 “이게 우리 둘만의 문제일까?”라는 생각이 들었습니다. 다른 친구끼리 만드는 팀, 학생, 소규모 팀, 인디 개발자들도 비슷한 불편을 겪을 수 있습니다. 그래서 TeamForge를 둘만 쓰는 개인 도구로 남겨두는 대신, 다른 사람들도 **소스를 확인하고, 개선하고, 테스트하고, 언젠가 실제로 사용할 수 있는 프로젝트**로 발전시켜 보기로 했습니다.
