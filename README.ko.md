...
TeamForge는 **사람이 방향을 정하고, 명세를 중심으로 AI 에이전트를 활용하는(human-directed, spec-driven, agentic) 방식**으로 AI의 도움을 상당히 많이 받아 개발되고 있습니다.

저는 주로 Project intent, 목표, 원하는 동작, 제약사항, 피드백, 테스트 결과와 증거, 최종 결정을 제공합니다. AI 도구는 그 입력을 더 자세한 요구사항, 설계 선택지, 구현, 테스트, 분석, 문서로 구체화하는 데 크게 관여합니다. 실제로는 거친 아이디어나 문제를 자연어로 표현한 뒤 AI와 함께 기술적인 명세로 세분화하고, 제가 검토하거나 방향을 다시 잡고, AI가 구현한 뒤 실제/자동 테스트 결과를 다시 다음 반복에 반영하는 식입니다.

저는 숙련된 전문 프로그래머라고 자신을 소개하지 않습니다. 모든 코드를 직접 손으로 작성하거나 전문적으로 리뷰했다고 오해받고 싶지도 않습니다. 이 설명은 개발 과정을 설명하는 것이며, 모든 코드 한 줄, 기술적 설계 표현, 문서를 제가 직접 작성했다는 주장이 아닙니다. TeamForge에는 AI 보조 및 AI 생성 자료가 상당히 포함되어 있습니다.
