(() => {
  ...
  ready(async () => {
    ...
    class Editor {
      ...
      render(){this.resize();this.orbit.update();if(this.selectionBox)this.selectionBox.update();if(this.remoteBox)this.remoteBox.update();this.renderer.render(this.scene,this.camera);}
    }
    ...
    const resizeObserver=new ResizeObserver(()=>site.editors.forEach((e)=>e.resize()));site.editors.forEach((e)=>resizeObserver.observe(e.sceneEl));const intersectionObserver=new IntersectionObserver((entries)=>{site.running=entries.some((entry)=>entry.isIntersecting);},{rootMargin:'300px 0px'});intersectionObserver.observe(demo);
