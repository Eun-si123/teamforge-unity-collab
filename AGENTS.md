...
- **Preserve existing work.** When a local Git worktree is available, inspect the current branch/status before meaningful file mutations. Treat pre-existing tracked or untracked changes as protected input: do not discard, overwrite, reset, restore, clean, stash, amend, rebase, force-push, or otherwise rewrite user/collaborator work unless the task explicitly requires that operation and its scope is understood. Do not silently include unrelated pre-existing changes in your own commit.
