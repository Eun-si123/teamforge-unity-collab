...
Ten separately named public-backfill repacks and ten `.sha256` sidecars were uploaded to the existing `TeamForge` Historical Builds prerelease on 2026-09-10. These are **new byte-level artifacts** and must not be treated as byte-identical replacements for historical originals.

Five of the published repacks were prepared from exact historical inner ZIPs recovered from preserved Codex outputs. The WP3.5 candidate and WP5 candidate were likewise individually recovered and verified before preparing public repacks. The WP4 and WP4.1 public repacks were derived from preserved privacy-sanitized archival copies. The Hotfix1 public repack was derived from the preserved Drive copy described above, while the older exact historical whole-archive bytes remain unrecovered. These distinctions are intentional: public archival coverage does not upgrade the evidence grade of the underlying historical byte identity.
...
| `Unity-TeamForge-Phase3-v0.4.1-hotfix1-candidate-privacy-sanitized-repack.zip` | `75C3F55652E83828F2AC6E950A04681E882A16024BD4C2750D221A27385421B4` |
...
For Hotfix1, the preserved Drive copy was verified as a structurally valid 176-entry ZIP at `376,058` bytes with SHA-256 `E6B7F4B7FEE6D80E7E9CFE8EC916681D32EB81286AA04D92CABE2EFCB3C5F610`. The public archival repack normalizes remaining work-state machine/path fixtures to generic `<WORKSPACE>` placeholders. The resulting `377,138`-byte ZIP has SHA-256 `75C3F55652E83828F2AC6E950A04681E882A16024BD4C2750D221A27385421B4`, contains 176 entries, passes ZIP integrity validation, and passed the same high-signal scan with no `/mnt/data/`, `/home/oai/`, real Windows user path, Tailscale `100.x` address, PEM private-key block, GitHub/OpenAI/AWS/Slack token pattern, or literal bearer-authorization material. RFC1918 documentation/test examples remain intentionally preserved as examples. The GitHub Release reports the exact same ZIP digest. This public repack does not make the unrecovered `9DE6...024F` original whole-archive byte stream recovered.

GitHub-reported digest verification now covers **20/20 backfill assets** total — ten ZIPs and ten `.sha256` sidecars. The previously verified nine ZIP/sidecar pairs remain unchanged, and the Hotfix1 ZIP/sidecar pair is present in the Historical Builds release with the expected published identity.

A complete release-inventory audit identified 51 preserved release-like ZIP candidates. After the Hotfix1 archival publication, public coverage resolves to **41 exact-name Release ZIPs + 10 separately named privacy-sanitized repacks + 0 unmatched publication gaps**. This is a publication-coverage result, not a claim that every historical original byte stream survives; Hotfix1 remains the explicit original-byte exception described above.
