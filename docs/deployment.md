...
On Windows, TeamForge-managed firewall onboarding uses only two named inbound rules: the current Coordinator TCP port and the actual bound Seed TCP port. Both are limited to the Private profile and `LocalSubnet`; changing the Seed port replaces the named Seed rule instead of accumulating old ports. **Remove LAN firewall rules when Host stops** is enabled by default: after TeamForge proves its Host lifecycle is idle, it removes those two named rules. Turn the option off to keep the narrow rules for faster restart; the listeners still close, but another program that later binds the same port could benefit from the retained inbound allowance. Automatic cleanup cannot run after an abrupt Unity/Windows termination. **Manual connection settings → Remove TeamForge LAN firewall rules** remains available after Host stop, and administrator approval is requested only when named rules are actually present.
