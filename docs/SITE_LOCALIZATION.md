...
The configured set is `en`, `ko`, `ja`, `zh-Hans`, `es`, `de`, `fr`, `pt-BR`, `zh-Hant`, `it`, `pl`, `tr`, `id`, `vi`, `ru`, and `ar`. Do not add another locale without a separate scope and maintenance decision. English and Korean retain their maintained lifecycle; the other fourteen languages remain public, non-indexable previews. Arabic uses the shared `direction: rtl` rendering path. Translation assistance and semantic comparison do not establish comprehensive native-speaker review.
