...
- Hardened Windows LAN hosting so the Seed remembers its last exact port (default `5091`), falls back to one OS-assigned port when that preferred port is occupied, and reconciles one named exact Seed firewall rule rather than accumulating port exceptions; the Advanced UI now defaults to removing the two named TeamForge LAN rules after a clean Host stop, exposes an on/off preference plus an info explanation, and retains explicit manual removal.
