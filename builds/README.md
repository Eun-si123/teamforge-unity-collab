...
- A privacy sanitization, metadata correction, or other repack therefore produces a replacement artifact with a new SHA-256. Do not describe two different archive hashes as the same immutable artifact.
...
- SHA-256: use the hash published with the **current privacy-sanitized candidate asset** in the `v0.5.1-prealpha-wp5.1` GitHub Release notes/sidecar. The filename alone is not sufficient artifact identity.
